...
typedef enum { RECEIVER,EMITTER } robot_types;
...
int main(int argc, char **argv)
{
  ...
  if (strcmp(wb_robot_get_name(), "vehicle1") == 0) {
    robot_type = EMITTER;
    printf("In the emitter\n");
    ...
    if (channel != COMMUNICATION_CHANNEL) {
      printf("not a right communication channel\n");
 ...
 // wb_distance_sensor_enable(ds0, TIME_STEP);
  ...
  // while(wb_robot_step(TIME_STEP)!=-1) {
    ...
    //if (robot_type == EMITTER) {
     ...
     /* printf("In the robot type emitter\n");
      const char *message = "Hello!";
      wb_emitter_send(communication, message, strlen(message) + 1);
      if(wb_emitter_send(communication, message, strlen(message) + 1) == 0)
      {
        printf("message sent atleast\n");
      }
    } else {*/
      /* is there at least one packet in the receiver's queue ? */
     // printf("the queue length is %d\n",wb_receiver_get_queue_length(communication));
      /*if (wb_receiver_get_queue_length(communication) > 0) {
      
      printf("com length greater than zero\n");*/
  ...
  while (wbu_driver_step() != -1) {
    ...
    if (i%(int)(TIME_STEP / wb_robot_get_basic_time_step()) == 0) {
      ...
      /* is there at least one packet in the receiver's queue ? */
         printf("the drver step in else is %d\n",wbu_driver_step());
         printf("the queue length is %d\n",wb_receiver_get_queue_length(communication));
        if (wb_receiver_get_queue_length(communication) > 0) {
